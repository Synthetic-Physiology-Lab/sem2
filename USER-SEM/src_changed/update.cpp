/* ----------------------------------------------------------------------
   ...
   ## Edited to incorporate SEM functionalities
------------------------------------------------------------------------- */
...
void Update::set_units(const char *style)
{
  ...
  //@@@SEM
  } else if (strcmp(style, "cell") == 0) {
    // SI-derived units with um, ug, s:
    // mass = micrograms (ug = 10^-9 kg)
    // distance = micrometers (um = 10^-6 m)
    // time = seconds
    // energy = zepto-Joule (zJ = 10^-21 J)
    // velocity = micrometers/s (10^-6 m/s)
    // force = femto-Newton (fN = 10^-15 N)
    // torque = femto-Newton * micrometers (10^-21 Nm)
    // temperature = degrees K
    // pressure = milli-Pascals (mPa = 10^-3 Pa)
    // dynamic viscosity = milli-Pascal*second (10^-3 Pa*s)
    // charge = Coulombs
    // dipole = Coulombs*micrometers (10^-6 C*m)
    // electric field = zeptovolts/micrometer (10^-15 V/m) <-- ???
    // density = micrograms/micrometer^dim (10^(-9+6*dim) kg/m^dim)
    const double lscale = 1e-6;
    const double mscale = 1e-9;
    const double tscale = 1;
    const double inv_escale = tscale*tscale/(mscale*lscale*lscale);
    
    force->boltz = 1.3806504e-23 * inv_escale;
    force->hplanck = 6.62606896e-34 * inv_escale/tscale;
    force->mvv2e = 1.0;
    force->ftm2v = 1.0;
    force->mv2d = 1.0;
    force->nktv2p = 1.0;
    // not 100% sure about next two (how is it used? what about volts?)
    force->qqr2e = 8.9876e9 * tscale*tscale/(mscale*lscale*lscale*lscale);
    force->qe2f = 1.0;
    force->vxmu2f = 1.0;
    force->xxt2kmu = 1.0;
    force->e_mass = 0.0;    // not yet set
    force->hhmrr2e = 0.0;
    force->mvh2r = 0.0;
    force->angstrom = 1.0e-10 / lscale;
    force->femtosecond = 1.0e-15 / tscale;
    // not 100% sure about next one (how is it used? what about volts?)
    force->qelectron = 1.6021765e-19 * inv_escale;
