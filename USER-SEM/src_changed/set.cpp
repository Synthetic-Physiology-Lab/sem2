...
enum{TYPE,TYPE_FRACTION,TYPE_RATIO,TYPE_SUBSET,
     ...
     //@@@SEM
     ,POLYMERIZATION,CELL,CELLPART,DENS};
...
void Set::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    //@@@SEM
      ...
      set(CELL);
      iarg += 2;
    } else if (strcmp(arg[iarg],"cellpart") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else ivalue = utils::inumeric(FLERR,arg[iarg+1],false,lmp);
      if (!atom->sem_flag)
      ...
      if (!atom->sem_flag || !atom->semDens_flag)
        error->all(FLERR,"Cannot set this attribute for this atom style");
      set(DENS);
      iarg += 2;
    //@@@SEM-end
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    //@@@SEM
    else if (keyword == POLYMERIZATION) atom->p[i] = dvalue;
    else if (keyword == CELL) atom->cell[i] = ivalue;
    else if (keyword == CELLPART) atom->cellpart[i] = ivalue;
    else if (keyword == DENS) atom->dens[i] = dvalue;
    //@@@SEM-END
