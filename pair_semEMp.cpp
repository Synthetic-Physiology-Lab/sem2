...
PairSemEMp::PairSemEMp(LAMMPS *lmp) : Pair(lmp) {}
...
void PairSemEMp::compute(int eflag, int vflag)
{
...
//	printf("%d: x: %f, %f, %f; p: %f; cell: %d\n",i,x[i][0],x[i][1],x[i][2],p[i],cell[i]);
//    }
#endif
...
//	printf("touch i: %i - in list: %i \n",i,jnum);
	for (jj = 0; jj < jnum; jj++) {
		...
		if(icell == jcell){
#ifdef __SEM_DEBUG__
		    printf("         INTRA\n");// Sandipan change -- fixing mistake -- INTRA instead of INTER
#endif
		    
		    E = exp(rho[itype]-rhoScl[itype]*rsqSclP);
		    ...
		    // accumulate dens
		    if(rsq<cutDens2[itype])dens[i]+=1.0;
			...
			if(rsq<cutDens2[itype])dens[j]+=1.0;
		    }
...
#ifdef __SEM_DEBUG__
		    printf("         INTER\n");// Sandipan change -- fixing mistake -- INTER instead of INTRA
#endif
		    // Sandipan change for excluding interaction between nuclear particles of one cell with particles of other cells starts
		    if (itype!=2 && jtype !=2){
		        E = exp(rho_inter[itype][jtype]-rhoScl_inter[itype][jtype]*rsqSclP);
		        fpair = factor_lj * semp1_inter[itype][jtype]*(2.*E*E-alpha_inter[itype][jtype]*E) * pFac;
		    }else{
		        E=0;
		        fpair=0;
		    }
		    // Sandipan change for excluding interaction between nuclear particles of one cell with particles of other cells ends
		    f[i][0] += delx*fpair;
		    f[i][1] += dely*fpair;
		    f[i][2] += delz*fpair;
		    if (newton_pair || j < nlocal) {
...
void PairSemEMp::allocate()
{
    ...
    // dens
    memory->create(cutDens2,n,"pair::cutDens2");
...
void PairSemEMp::settings(int narg, char **arg)
{
...
void PairSemEMp::coeff(int narg, char **arg)
{
    int ii,jj;
    ...
    double densFac_one = utils::numeric(FLERR,arg[9],false,lmp);
    ...
    r0[cellType]=r0_one;
    // dens
    cutDens2[cellType]=r0_one*r0_one*densFac_one*densFac_one;
    ...
    printf("** pair_semEMp: set coeffs: u0 %f, u0_inter %f, rho0 %f, rho0_inter %f, alpha %f, alpha_inter %f, r0 %f, cutdens^2 %f\n",
	   u0[cellType],u0_inter[cellType][cellType],rho[cellType],rho_inter[cellType][cellType],alpha[cellType],alpha_inter[cellType][cellType],r0[cellType],cutDens2[cellType]);
...
double PairSemEMp::init_one(int i, int j)
{	   
    printf("** pair_semEMp: init_one for %d and %d\n",i,j);
    double r02;
...
void PairSemEMp::write_restart(FILE *fp)
{
...
/* ----------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------- */
//@@@SEM RESTART NOT TESTED
void PairSemEMp::read_restart(FILE *fp)
{
...
void PairSemEMp::write_restart_settings(FILE *fp)
{	
...
void PairSemEMp::read_restart_settings(FILE *fp)
{
    if (comm->me == 0) {
...
double PairSemEMp::single(int i, int j, int itype, int jtype, int icell, int jcell,double poly, double rsq,
